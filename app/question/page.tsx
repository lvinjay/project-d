        ...
        <div className="card formCard">
          ...
          <div className="field">
            ...
            <select className="selectInput" value={budget} onChange={(event) => setBudget(Number(event.target.value))}>
              <option value={300000}>30만원</option><option value={500000}>50만원</option><option value={700000}>70만원</option><option value={1000000}>100만원</option><option value={1500000}>150만원</option><option value={0}>가격 제한 없음</option>
            </select>
          </div>
...
function QuestionLoading() {
  return <main><Header /><section className="container emptyState"><h1>구매 조건 화면을 불러오는 중입니다.</h1><p className="sectionLead">잠시만 기다려 주세요.</p></section></main>;
}

export default function QuestionPage() {
  return <Suspense fallback={<QuestionLoading />}><QuestionContent /></Suspense>;
}
