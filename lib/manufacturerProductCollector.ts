...
export async function collectManufacturerProduct(
  ...
  */
  ...
  const compactComparableDetailText =
    comparableDetailText.replace(
      /\s+/g,
      "",
    );

  let missingSearchTerms =
    normalizedSearchTerms.filter(
      (term) =>
        !compactComparableDetailText.includes(
          term,
        ),
    );

  /*
    일부 제조사 프로모션/시리즈 페이지는
    og:title / JSON-LD에는 브랜드명만 넣고
    실제 모델명은 HTML 본문에만 노출한다.

    단, 본문 전체를 무조건 identity 근거로 쓰면
    관련상품/메뉴에 등장한 다른 모델을 잘못 통과시킬 수 있다.

    따라서:
    - metadata/title/url 검증이 실패했고
    - 요청된 strong model token 전체가
      HTML 안에서 "하나의 연속된 모델 표현"으로 실제 존재할 때만
    보조 identity로 인정한다.
  */
  if (
    ...
    const requestedModelPhrase =
      normalizedSearchTerms.join("");

    if (
      requestedModelPhrase &&
      compactHtmlText.includes(
        requestedModelPhrase,
      )
    ) {
      missingSearchTerms = [];
      ...
      /*
        route 단계의 validateProductMatch()도
        detail.title / detail.modelName을 다시 검증한다.

        프로모션 페이지처럼 title이 "DREAME"이고
        modelName 메타데이터가 비어 있는 경우,
        본문에서 정확히 확인된 strong model token 조합을
        modelName 보조값으로 전달한다.

        본문에 요청 모델 표현 전체가 연속해서 존재할 때만
        이 값을 넣으므로 다른 시리즈/관련상품 오매칭은 막는다.
      */
      if (!detail.modelName) {
        detail.modelName =
          searchTerms.join(" ");
