...
async function readJson(
  ...
  try {
    ...
    throw new Error(
      `JSON이 아닌 응답입니다. (${response.status}) ${text.slice(
        0,
        200,
      )}`,
    );
    ...
    async function run() {
      try {
        ...
        if (
          ...
          throw new Error(
            `유효 후보가 5개에 도달하지 못했습니다. 현재 ${finalCandidates.length}개입니다.`,
          );
        }
        ...
        updateStep(
          "enrich",
          "done",
          `${finalCandidates.length}개 최종 후보 확보`,
        );
